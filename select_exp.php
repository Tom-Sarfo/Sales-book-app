<?php  
 ...
 $output = '';
 $query = "SELECT SUM(`amount_paid`) AS amount FROM `retail_orders` WHERE `acc_status`= 1";
        $res = mysqli_query($connect, $query);
        while($row = mysqli_fetch_assoc($res)) {
          $amt = $row['amount'];
        }
  $sql = "SELECT SUM(`amount_paid`) AS momo_amt FROM `retail_orders` WHERE `payment_mode`= 'Momo' AND `acc_status`= 1";
  $outcome = mysqli_query($connect, $sql);
  while($row = mysqli_fetch_assoc($outcome)) {
    $momo = $row['momo_amt'];
  }
  $sqli = "SELECT SUM(`amount_paid`) AS cash_amt FROM `retail_orders` WHERE `payment_mode`= 'Cash' AND `acc_status`= 1";
 ...
 <div class="jumbotron xs-4">
      ...
      <a href="index.php" align="right" class="btn btn-primary" style="text-decoration: none; color: #ffffff;">Go to Sales order</a>
  ...
  $sql = "SELECT * FROM retail_orders ORDER BY id DESC";  
